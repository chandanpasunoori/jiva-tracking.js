...
describe('Jiva.utils.cookie', function() {

  beforeEach(function(){
    this.cookie = cookie('jiva-test-cookie');
  });
  ...
  describe('constructor', function(){
    ...
    it('should have internal properties', function() {
      ...
      assert.equal(this.cookie.config.key, 'jiva-test-cookie');
    });
  ...
  describe('.get', function(){
    ...
    it('should return a simple string for pre-existing non-json data', function(){
      Cookies.set('jiva-test-cookie', 'some thing that is not json');
  ...
  describe('.set', function(){
    ...
    it('should set a string value', function(){
      this.cookie.set('library', 'jiva-tracking.js');
      assert.equal(this.cookie.get('library'), 'jiva-tracking.js');
    });
    ...
    it('should set an object of key:value pairs', function(){
      this.cookie.set({
        library: 'jiva-tracking.js',
        number: 123,
        array: ['1', 2, false],
        object: { object: true }
      });
      var data = this.cookie.get();
      assert.equal(data.library, 'jiva-tracking.js');
