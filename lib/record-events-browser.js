...
function recordEvent(eventCollection, eventBody, callback, async){
  ...
  if (!Jiva.enabled) {
    handleValidationError.call(this, 'Jiva.enabled is set to false.', cb);
    return false;
  }
...
function addEvent(){
  this.emit('error', 'This method has been deprecated. Check out #recordEvent: https://github.com/etsrepo/jiva-tracking.js.git#record-a-single-event');
  recordEvent.apply(this, arguments);
}

function addEvents(){
  this.emit('error', 'This method has been deprecated. Check out #recordEvents: https://github.com/etsrepo/jiva-tracking.js.git#record-multiple-events');
...
function checkValidation(callback){
  ...
  if (!this.projectId()) {
    handleValidationError.call(this, 'Jiva.Client is missing a projectId property.', cb);
    return false;
  }
  if (!this.writeKey()) {
    handleValidationError.call(this, 'Jiva.Client is missing a writeKey property.', cb);
    return false;
  }
  return true;
}
    ...
    if (xhr.readyState == 4) {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          Jiva.emit('error', 'Could not parse HTTP response: ' + xhr.responseText);
          if (cb) {
        ...
        Jiva.emit('error', 'HTTP request failed.');
        if (cb) {
          cb.call(self, xhr, null);
        }
      }
...
function sendJSONp(url, callback){
  var self = this,
      ...
      callbackName = 'jivaJSONPCallback',
      loaded = false;
