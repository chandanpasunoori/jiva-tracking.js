...
These methods handle an internal queue of events, which is pushed to the [events](https://jiva.io/docs/api/#record-multiple-events) API resource on a given interval (default: 15 seconds), or when the queue reaches a maximum capacity (default: 5000 events).
