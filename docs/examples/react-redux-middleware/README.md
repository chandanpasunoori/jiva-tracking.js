...
```ssh
redux/examples/real-world/src/store/jiva-middleware.js
```
...
import JivaMiddleware from './jiva-middleware';
...
const configureStore = preloadedState => {
  const store = createStore(
    ...
    compose(
      ...
      applyMiddleware(thunk, api, createLogger(), JivaMiddleware),
      DevTools.instrument()
    )
  );
...
const configureStore = preloadedState => createStore(
  ...
  preloadedState,
  // 2. Append to middleware composition
  applyMiddleware(thunk, api, JivaMiddleware)
)
